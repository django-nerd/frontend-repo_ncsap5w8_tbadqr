...
export default function Teachers() {
  ...
  const loadReport = async (id) => {
    const res = await fetch(`${API_BASE}/teachers/${id}/performance`, { headers: { Authorization: `Bearer ${tokenRef.current}` } });
    const data = await res.json();
    setReport(data);
  };
  ...
  return (
    <div className="grid gap-6">
      <h1 className="text-2xl font-semibold">Teachers Performance</h1>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="rounded-xl bg-white/5 border border-white/10 p-4">
          ...
          <ul className="space-y-2">
            {results.map(t => (
              <li key={t._id}>
                <button onClick={() => loadReport(t._id)} className="w-full text-left px-3 py-2 rounded-lg hover:bg-white/5">
                  {t.first_name} {t.last_name} <span className="text-blue-300/70 text-sm">{t.subject}</span>
                </button>
              </li>
            ))}
        ...
        <div className="rounded-xl bg-white/5 border border-white/10 p-4 print:bg-white print:text-black">
          {report ? (
            <div>
              ...
              <div className="text-blue-200 mb-2">{report.teacher.first_name} {report.teacher.last_name}</div>
          ...
          ) : <div className="text-blue-300/70">Select a teacher to view report</div>}
        </div>
      </div>
    </div>
  );
